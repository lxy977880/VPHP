...
class SphinxApi
{
	...
	function __construct ($_host,$_port="")
	{
		...
		$this->_port = (intval($_port) > 0) ? $_port : 3312;
	...
	function AddQuery ( $query, $index="*", $comment="" )
	{
		...
		$req .= self::sphPack64 ( $this->_min_id ) . self::sphPack64 ( $this->_max_id ); // id64 range
	...
	function RunQueries ()
	{
		...
		for ( $ires=0; $ires<$nreqs && $p<$max; $ires++ )
		{
			...
			while ( $count-->0 && $p<$max )
			{
				...
				if ( $id64 )
				{
					$doc = self::sphUnpack64 ( substr ( $response, $p, 8 ) ); $p += 8;
